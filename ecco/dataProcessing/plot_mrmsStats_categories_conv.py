...
def main():
    ...
    infile='mrmsStatsConvectivity_20220501_to_20220501.pickle'
    infile2='mrmsStatsConvectivity_20220501_to_20220501.pickle'
    
    figdir='/scr/cirrus2/rsfdata/projects/nexrad-mrms/figures/eccoStats/'
    
    with open(indir+infile, 'rb') as handle:
        conv2D=pickle.load(handle)
        
    with open(indir+infile2, 'rb') as handle:
        echoType2D=pickle.load(handle)
    
    xlims=[min(conv2D['lon']),max(conv2D['lon'])]
    ylims=[min(conv2D['lat']),max(conv2D['lat'])]
    ...
    plt.savefig(figdir+'categoriesConvectivity_'+infile[-27:-7]+'.png')
