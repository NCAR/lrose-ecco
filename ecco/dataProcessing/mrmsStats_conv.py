...
def main():
    ...
    for readFile in fileList:
        ...
        conv2Din=np.squeeze(readThis.variables['ConvectivityComp'][:])
        #conv2Din=conv2Din.filled(fill_value=np.nan)
        
        for ii in range(24):
            #print(ii)
            convThis=np.where(((echoType2Din==14) & (mstHours2D==ii)),conv2Din,0);
            StratLow[ii,:,:]=StratLow[ii,:,:]+convThis
            convThis=np.where(((echoType2Din==16) & (mstHours2D==ii)),conv2Din,0);
            StratMid[ii,:,:]=StratMid[ii,:,:]+convThis
            convThis=np.where(((echoType2Din==18) & (mstHours2D==ii)),conv2Din,0);
            StratHigh[ii,:,:]=StratHigh[ii,:,:]+convThis
            convThis=np.where(((echoType2Din==25) & (mstHours2D==ii)),conv2Din,0);
            Mixed[ii,:,:]=Mixed[ii,:,:]+convThis
            convThis=np.where(((echoType2Din==32) & (mstHours2D==ii)),conv2Din,0);
            ConvElev[ii,:,:]=ConvElev[ii,:,:]+convThis
            convThis=np.where(((echoType2Din==34) & (mstHours2D==ii)),conv2Din,0);
            ConvShallow[ii,:,:]=ConvShallow[ii,:,:]+convThis
            convThis=np.where(((echoType2Din==36) & (mstHours2D==ii)),conv2Din,0);
            ConvMid[ii,:,:]=ConvMid[ii,:,:]+convThis
            convThis=np.where(((echoType2Din==38) & (mstHours2D==ii)),conv2Din,0);
            ConvDeep[ii,:,:]=ConvDeep[ii,:,:]+convThis
    ...
    echoType2D={
        ...
        "lon":lon,
        "lat":lat,
        "StratLowConvectivity":StratLow,
        "StratMidConvectivity":StratMid,
        "StratHighConvectivity":StratHigh,
        "MixedConvectivity":Mixed,
        "ConvElevConvectivity":ConvElev,
        "ConvShallowConvectivity":ConvShallow,
        "ConvMidConvectivity":ConvMid,
        "ConvDeepConvectivity":ConvDeep
    }
    
    outfile=options.outDir+'mrmsStatsConvectivity_'+startTime.strftime("%Y%m%d")+'_to_'+endTime.strftime("%Y%m%d")+'.pickle'
